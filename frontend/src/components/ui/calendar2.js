      ...
      days.push(
        ...
        >
          ...
          {dayEvents.length > 0 && (
            <div className="flex gap-0.5 mt-0.5">
              {dayEvents.slice(0, 3).map((event, idx) => (
                <span
                  key={idx}
                  className={`w-1.5 h-1.5 rounded-full ${isToday ? 'bg-white/70' : (eventTypeColors[event.type] || 'bg-gray-400')}`}
                />
              ))}
            </div>
          )}
          ...
          {hoveredDay === day && dayEvents.length > 0 && (
            <div className="absolute z-30 left-1/2 -translate-x-1/2 top-full mt-1 bg-white border border-gray-200 shadow-lg rounded-lg p-2.5 text-xs min-w-[150px] pointer-events-none animate-fade-in">
              {dayEvents.map((event, idx) => (
                <div key={idx} className="flex items-center gap-2 mb-1 last:mb-0">
                  <span className={`w-2 h-2 rounded-full flex-shrink-0 ${eventTypeColors[event.type] || 'bg-gray-400'}`} />
                  <span className="text-gray-700 font-medium">{eventTypeLabels[event.type] || event.type}</span>
                </div>
              ))}
            </div>
          )}
