export default function DaysOffSummary({ daysOffData }) {
    if (!daysOffData) {
      return (
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="text-sm font-semibold mb-3 text-blue-800">Días Libres</h3>
          <p className="text-sm text-gray-500">No hay datos disponibles</p>
        </div>
      );
    }
  
    const { total, taken, remaining, nextDayOff } = daysOffData;
